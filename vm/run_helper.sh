...
# Config
ETISS_DIR=/data/home.local/repos/vector_extension_tests/onetiss/sim/etiss/build/installed/
...
# Call
ARGS="-ibase.ini -i${DYN_INI}"
if [ "${USE_TGDB}" -eq 1 ]; then
    if [ "${DO_NOT_ATTACH}" -eq 0 ]; then
        konsole --workdir $(pwd) -e "bash -c '${RISCVV}/bin/riscv64-unknown-elf-gdb -ex \"tar rem :2222\" $TARGETSW_PATH_ELF'" &
    fi
fi
